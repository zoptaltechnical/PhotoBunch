...
#define FTDefaultMenuWidth          70.0
...
+ (void) showForSender:(UIView *)sender
        ...
        imageNameArray:(NSArray<NSString*> *)imageNameArray
          ...
          dismissBlock:(FTPopOverMenuDismissBlock)dismissBlock;
{
    if ([[menuArray objectAtIndex:0]length]==0)
    {
...
- (instancetype)init
{
    self = [super init];
    if (self) {
        [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(onChangeStatusBarOrientationNotification:)  name:UIApplicationDidChangeStatusBarOrientationNotification
                                                   object:nil];
    }
    return self;
}
